...
- [x] Tela de login + rotas protegidas (redireciona para `/login` sem token,
...
- [x] CRUD completo de produtores e propriedades (criar/listar/editar/
      remover — inclui edição, que antes só existia na API)

- [x] Telas de Safras e Culturas — CRUD completo (`update`/`remove` com
...
- [x] Plantio (safra + cultura) selecionável já no cadastro da propriedade
      (persistido logo após a criação) e editável depois no modal de edição

- [x] Dashboard com gráficos donut (estado, cultura, uso do solo) — clicar
...
- [x] Listagem de propriedades em grid de cards com mapa real
...
- [x] Seleção de UF e cidade com busca (autocomplete), em vez de `<select>` nativo

- [x] Avatar com iniciais coloridas (determinístico por nome) na listagem
      de produtores + máscara de CPF/CNPJ (exibição e digitação)

- [x] Toasts de sucesso/erro em toda ação de criar, editar e remover, e nos
      campos obrigatórios não preenchidos

- [x] Validação visual de campos obrigatórios em todos os formulários
      (login, produtor, propriedade, safra/cultura, plantio)

- [x] Layout responsivo (sidebar vira barra superior em telas ≤900px,
      formulários colapsam para 1 coluna em telas ≤640px, menu hambúrguer)
